#!/bin/bash
# Full end-to-end validation of the actual proposed upstream patch, run
# against the ENTIRE, UNMODIFIED (except for the patch) upstream
# crates/ostree-ext/ci/priv-integration.sh -- not the trimmed two-section
# stand-in used by repro-patched-priv-integration.sh.
#
...
# (the same commit repro-patched-priv-integration.sh was validated
# against), with ONLY the 4 `systemd-run -dP --wait ` prefixes deleted
# (lines 109, 112, 113, 153 of the original). Nothing else was changed.
#
# Purpose: the earlier repro-patched-priv-integration.sh deliberately
# skipped the ~100 lines of `ostree admin`/`ostree container image`
# sysroot setup at the top of the script (upstream lines 1-102, 115-150),
# on the theory that they have no systemd-run call sites and don't touch
# dbus/systemd. That's true, but those lines DO exercise
# ostree_sysroot_stage_tree_with_options's systemd-run/dbus dependent
# code path *if* the sysroot is ever considered "booted" -- which
# shouldn't happen here since sysroot=/run/host is a bind-mounted view of
# the host's root, not the container's own /. This script runs the WHOLE
# thing end to end to confirm that empirically rather than by source
# reading alone. See AGENTS.md for the full investigation.
set -euo pipefail
