#!/bin/bash
# Full end-to-end validation of the CORRECTED proposed upstream patch
# (bootc PR #2392, commit 6568a52b as of this writing), run against the
# ENTIRE, UNMODIFIED (except for the patch) upstream
# crates/ostree-ext/ci/priv-integration.sh.
#
# This supersedes priv-integration-patched-full.sh, which validated an
# earlier version of the patch that dropped BOTH /run/dbus and
# /run/systemd from the outer podman invocation and simply deleted the 4
# `systemd-run -dP --wait` prefixes. That version fixed the chunkah hang
# (see repro-chunkah-no-host-systemd.yml) but broke the `nsenter -m -t 1
# journalctl _COMM=bootc` check a few lines up: ostree-ext's structured
# logging silently skips writing to the journal unless
# libsystemd::daemon::booted() (a check for /run/systemd/system) is true
# (see repro-patched-priv-integration-full.yml /
# -full-systemd-control.yml for how that was found).
#
# The corrected patch keeps /run/systemd mounted (drops only /run/dbus,
# never needed) so the journal check keeps working, then has the script
# itself `umount /run/systemd` right after that check, before any of the
# systemd-run-free podman/build work later in the script. This was
# validated in isolation at the chunkah-only level in
# repro-chunkah-unmount-systemd.yml (run 32406651261): cgroup delegation
# for nested podman is decided dynamically per invocation based on
# *current* /run/systemd presence, not baked in once at outer-container
# creation time, so the mid-script unmount is sufficient and the nested
# chunkah podman run stays in the outer container's own namespaces
# afterward (no host-namespace escape).
#
# This is a byte-for-byte copy of crates/ostree-ext/ci/priv-integration.sh
# as of bootc-dev/bootc commit fceedf69901f8e0ede49d9ce1974a76a39dd3eb1,
# with exactly the diff from bootc PR #2392 commit 6568a52b applied (the 4
# `systemd-run -dP --wait ` prefixes deleted, plus the `umount
# /run/systemd` line added after the journal check). Nothing else was
# changed.
set -euo pipefail
